<script lang="ts">
	...
	// Toppings state
	const toppings = ['Pepperoni', 'Mushrooms', 'Onions', 'Sausage', 'Green Peppers'];
	...
	function handleTodoChange(id: number, checked: boolean) {
		todoList = todoList.map((todo) => (todo.id === id ? { ...todo, done: checked } : todo));
	}
...
<div class="space-y-4">
				...
				<div>
					...
					<div class="flex flex-wrap gap-4">
						<Radio
							...
							onCheckedChange={() => (selectedColor = 'default')}
						/>
						<Radio
							name="colors"
							value="neutral"
							...
							onCheckedChange={() => (selectedColor = 'neutral')}
						/>
						<Radio
							name="colors"
							value="primary"
							...
							onCheckedChange={() => (selectedColor = 'primary')}
						/>
						<Radio
							name="colors"
							value="secondary"
							...
							onCheckedChange={() => (selectedColor = 'secondary')}
						/>
						<Radio
							name="colors"
							value="accent"
							...
							onCheckedChange={() => (selectedColor = 'accent')}
						/>
						<Radio
							name="colors"
							value="info"
							...
							onCheckedChange={() => (selectedColor = 'info')}
						/>
						<Radio
							name="colors"
							value="success"
							...
							onCheckedChange={() => (selectedColor = 'success')}
						/>
						<Radio
							name="colors"
							value="warning"
							...
							onCheckedChange={() => (selectedColor = 'warning')}
						/>
						<Radio
							name="colors"
							value="error"
							...
							onCheckedChange={() => (selectedColor = 'error')}
						/>
					</div>
					{#if selectedColor}
		...
		<CardBody>
			...
			<div class="space-y-8">
				...
				<div>
					...
					<Range value={rangeValue} onValueChange={(v) => (rangeValue = v)} showFloatingLabel />
		...
		<CardBody>
			...
			<div class="space-y-8">
				...
				<div>
					...
					<Rating value={3.5} readonly allowHalf color="bg-green-500" />
				</div>
			</div>
		</CardBody>
	</Card>
		...
		<CardBody>
			...
			<div class="space-y-8">
				...
				<div>
					...
					<Select disabled options={['You', "can't", 'touch', 'this']} value="You" />
				</div>
			</div>
		</CardBody>
	</Card>
	...
	<Card>
		<CardBody>
			...
			<div class="space-y-4">
				<FileInput label="Choose a file" placeholder="Select file..." />

				<fieldset class="fieldset">
					<legend class="fieldset-legend">Upload Document</legend>
					<FileInput label="Max size 2MB" labelPosition="after" accept=".pdf,.doc,.docx" />
				</fieldset>
				...
				<FileInput disabled label="Disabled input" placeholder="You can't touch this" />
			</div>
		</CardBody>
	</Card>
		...
		<CardBody>
			...
			<div class="space-y-8">
				...
				<div>
					...
					<Textarea placeholder="Enter your bio..." bind:value={bioText} rows={4} />
					{#if bioText}
				...
				<div>
					...
					<div class="grid gap-4 md:grid-cols-2">
						{#each colors as color}
							<Textarea {color} placeholder={`${color} textarea`} rows={2} />
						{/each}
					</div>
				</div>
				...
				<div>
					...
					<div class="space-y-4">
						{#each sizes as size}
							<Textarea {size} placeholder={`${size} size textarea`} />
						{/each}
					</div>
				</div>
				...
				<div>
					...
					<fieldset class="fieldset p-4 bg-base-100 border border-base-300 rounded-box w-64">
						...
						<label class="fieldset-label flex items-center gap-2 cursor-pointer">
							<Toggle bind:checked={themeToggle} color="primary" />
							Dark mode
						</label>
					</fieldset>
				</div>
